...
е) Построить график ```scatter()```, где по оси x - год выпуска, по оси y - пробег в милях (в 1 км 0.621371 миль)

ж) Построить график ```scatter()```, где по оси x - пробег в милях, а по оси y - цена в _тысячах_ рублей

з) Построить гистограмму с ценами всех авто

и) Построить столбчатую диаграмму, где каждый столбец - это количество авто за каждый год

к) Добавить интерактивный элемент с помощью ```ipywidgets```, чтобы для какого-то из графиков выше можно было делать фильтр по названию модели и типу коробки передач
